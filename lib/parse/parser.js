...
import { extractPostDigest } from './parserBase.js'
...
export function extractPostInfo(dataObj, url) {
  ...
  // 提取正文：多级回退（F-08），共享逻辑已迁移至 parserBase.extractPostDigest
  const digest = extractPostDigest(postView, hasImages)
...
export function extractImageUrl(link) {
  const imgUrl = link.orign || link.raw
