 ...
 * @param {string} [_sort='new'] - 排序方式（保留参数位以兼容调用方签名，当前未在函数内使用）
 * @returns {{ items: TagPagePost[], offset: number }} 解析后的帖子列表和下一页偏移
 */
export function parseAPIResponse(apiData, tagName, _sort = 'new') {
  ...
  const items = list.map((item) => {
    ...
    return {
      title: postView.title || '无标题',
      digest: cleanDigestText(postView.digest),
      type: resolvePostType(postView.type),
      blogNickName: blogInfo.blogNickName || '未知',
      publishTime: postView.publishTime || 0,
      photoCount: postView.photoCount || 0,
