...
import { downloadImage } from './fetcher.js'
import { extractImageUrl, extractImageExt } from './parser.js'
import { formatDate, sanitizeFileName, THUMBNAIL_TRANSFORM, sleep } from './utils.js'
...
export function resolveImageTarget(photoLink, index, totalCount, context) {
  ...
  let imgUrl = extractImageUrl(photoLink)
  if (!imgUrl) return null

  // 作品保护时使用缩略图
  if (context.imageProtected) {
    imgUrl = `${imgUrl}${THUMBNAIL_TRANSFORM}`
  }

  const fileName = buildImageFileName(blogger.blogName, post.publishTime, index, totalCount, imgUrl)
  return { imgUrl, fileName, filePath: path.join(tempDir, fileName) }
}
...
export async function processImage(photoLink, index, totalCount, context) {
  ...
  const { imgUrl, fileName, filePath } = target
  const { config, post } = context

  // 下载 + 1 次重试（F-02）
  const downloadResult = await downloadWithRetry(imgUrl, post.url, { tempDir: context.tempDir, fileName })
  if (!downloadResult) {
  ...
  if (cls.branch === 'thumbnail') {
    // 缩略图改为本地下载（P-06）
    const localThumb = await downloadThumbnailLocally(cls.thumbnailUrl, context.tempDir, fileName)
    return {
      success: true,
      isThumbnail: true,
      imgUrl,
 ...
 */
async function downloadThumbnailLocally(thumbnailUrl, tempDir, originFileName) {
  try {
