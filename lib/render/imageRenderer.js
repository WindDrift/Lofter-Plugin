...
export function resolveFontConfig(customFontName = '') {
  ...
  if (fs.existsSync(fontsDir)) {
    ...
    if (customFontName) {
      localFontFile =
        fontFiles.find((f) => path.basename(f, path.extname(f)) === customFontName) ||
...
export async function renderTextAsImages(params) {
  ...
  const resolvedTotalTextCount =
    totalTextCount ?? paragraphs.reduce((sum, paragraph) => sum + countTextUnits(paragraph), 0)
