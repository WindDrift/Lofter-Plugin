...
/**
 * 统一列表消息发送（合并转发或逐条发送）
 * 支持 ListMessageItem 对象（{ text, thumbnailUrl }），有缩略图时会附加 segment.image
 * @param {object} e - 云崽消息事件对象
 * @param {Array<string|ListMessageItem>} messages - 消息列表
 * @param {object} config - 配置对象
 */
export async function sendListResult(e, messages, config) {
  const msgList = messages.map((msg) => {
    if (msg && typeof msg === 'object' && msg.thumbnailUrl) {
      return [segment.image(msg.thumbnailUrl), msg.text]
    }
    return msg?.text || msg
  })
  ...
  if (config.sendMode === 'forward') {
    const forwardMsg = await makeForwardMsg(
      e,
      msgList,
      config.forwardTitle || 'Lofter解析结果',
      config.forwardNickname || ''
    )
    if (forwardMsg) {
