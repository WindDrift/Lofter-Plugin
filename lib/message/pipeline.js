...
import { fetchPage, fetchBlogHomePageByAPI, cleanupTempFiles, cleanupFile } from '../fetch/fetcher.js'
import { setListCache } from '../fetch/listCache.js'
import { parsePageData, extractPostInfo, extractImageUrl } from '../parse/parser.js'
import { extractCollectionInfo } from '../parse/collectionParser.js'
...
export async function executeParsePipeline(e, { url, config, recordParse, onDeveloperMode }) {
  ...
  try {
    ...
    // Step 2.5: 通过移动端 API 获取合集信息（失败不影响主流程）
    postInfo.collection = await stepFetchCollectionInfo({ blogger: postInfo.blogger, post: postInfo.post, config })

    const { blogger, post, interaction, collection } = postInfo
    const postWithTime = {
      ...post,
    ...
    // Step 5: 组装文本消息
    const textMessages = buildTextMessages({
      blogger,
      post: postWithTime,
      interaction,
      collection,
    ...
    const imageResult = await stepHandleImages({
      post,
      blogger,
      config,
      msgList,
      existingFirstImagePath: imageMode.firstImagePath
    })
    ...
    // 纯文图片模式：单独发送原文合并转发消息，避免在合并转发模式下被嵌套
    if (imageMode.originalTextForwardMsg) {
      await e.reply(imageMode.originalTextForwardMsg)
    }

    // 发送合集文章选择列表，方便用户快速解析同合集其他文章
    await stepSendCollectionList({ e, collection, blogger, config })
...
/** Step 2: 抓取并解析 */
async function stepFetchAndParse({ url, timeout, config }) {
  const html = await fetchPage(url, timeout, config)
  const dataObj = parsePageData(html)
  return extractPostInfo(dataObj, url)
}

/** Step 2.5: 获取合集信息（失败时返回 null，不阻塞主流程） */
async function stepFetchCollectionInfo({ blogger, post, config }) {
  if (!blogger?.blogName || !post?.postId) return null
  try {
    const homePageResponse = await fetchBlogHomePageByAPI(blogger.blogName, config)
    return extractCollectionInfo(homePageResponse, post.postId)
  } catch (err) {
    logger.debug?.(`[Lofter解析] 获取合集信息失败: ${err.message}`)
    return null
  }
}
...
function stepProcessText({ post, config }) {
  ...
  const expectedImageCount =
    !post.hasImages && pureTextSendMode === 'image'
...
/** Step 5: 组装文本消息 */
function buildTextMessages({ blogger, post, interaction, collection, paragraphs, config }) {
  const messages = []
  ...
  if (config.sendCollectionInfo && collection) messages.push(buildCollectionMessage(collection))
...
/** Step 6: 纯文图片模式渲染 */
async function stepRenderImageMode({ e, post, blogger, config, textCtx, textMessages }) {
  if (!config.sendPostBody || post.hasImages || (config.pureTextSendMode || 'single') !== 'image') {
    return { firstImagePath: null, isImageMode: false, originalTextForwardMsg: null }
  }
  let firstImagePath = null
  let originalTextForwardMsg = null
  try {
    ...
    if (imagePaths && imagePaths.length > 0) {
      ...
      // 图片后追加原文合并转发消息，方便用户直接复制
      const originalText = post.title
        ? `${post.title}\n\n${textCtx.paragraphs.join('\n\n')}`
        : textCtx.paragraphs.join('\n\n')
      const splitOriginalTexts = splitTextByLength(originalText, 2000)
      originalTextForwardMsg = await makeForwardMsg(
        e,
        splitOriginalTexts,
        'Lofter原文',
        blogger.nickname || config.forwardNickname || ''
      )
    } else {
  ...
  return { firstImagePath, isImageMode: true, originalTextForwardMsg }
}
...
async function stepHandleImages({ post, blogger, config, msgList, existingFirstImagePath }) {
  ...
  const ctxFor = (_i) => ({ blogger, post, config, tempDir, imageProtected: blogger.imageProtected })
  ...
  if (blogger.imageProtected && result.successImageCount > 0) {
    const firstImageIdx = msgList.findIndex(
      (msg) => msg?.type === 'lofter-image' || (Array.isArray(msg) && msg.some((m) => m?.type === 'image'))
    )
    if (firstImageIdx >= 0) {
...
function dispatchImageResult({ r, i, config, msgList, result }) {
  ...
  msgList.push({
    ...
    filePath: r.filePath,
    fileName: r.fileName,
    originMsg: config.sendImageLinks ? originMsg : ''
  })
...
async function stepSendForward({ e, msgList, post, blogger, config, imageResult }) {
  ...
  try {
    const forwardMsg = await makeForwardMsg(
      e,
      msgList.map((msg) => toForwardMessage(msg)),
      forwardTitle,
      forwardNickname
    )
    if (forwardMsg) {
...
async function sendFirstImagePreview({ e, post, config, image, blogger }) {
  ...
  if (firstImagePath) {
    ...
    await e.reply(
      `解析成功${post.photoLinks.length}张图片，均超过设定的大小上限（${sizeLimitMB}MB），请点击合并转发查看具体链接。`
    )
  }
}
...
/**
 * 发送合集文章选择列表，并将列表写入缓存供 #lofter解析 序号 使用
 * @param {object} e - 消息事件对象
 * @param {CollectionInfo|null} collection - 合集信息
 * @param {BloggerInfo} blogger - 博主信息
 * @param {LofterConfig} config - 配置对象
 */
async function stepSendCollectionList({ e, collection, blogger, config }) {
  if (!collection || !collection.posts || collection.posts.length <= 1) return
  if (!config.sendCollectionList) return

  const limit = config.collectionListLimit || 10
  const listMsg = buildCollectionListMessage(collection, limit)
  if (!listMsg) return
  ...
  try {
    setListCache(
      ...
      {
        type: 'collection',
        items: collection.posts.map((post) => ({
          blogInfo: { blogName: blogger.blogName },
    ...
    await e.reply(listMsg)
  } catch (err) {
    logger.debug?.(`[Lofter解析] 发送合集列表失败: ${err.message}`)
  }
}
