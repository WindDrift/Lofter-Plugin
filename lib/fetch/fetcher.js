...
/**
 * 通过 DWR 接口获取标签页数据
 * @param {string} tagName - 标签名（已编码）
 * @param {string} sort - 排序方式 (new/hot/month/week/date/total)
 * @param {number} getNum - 每次获取条数，默认 20
 * @param {number} gotNum - 已获取条数，默认 0
 * @param {number} lastTimestamp - 最后一条的时间戳，默认当前时间戳
 * @param {LofterConfig} config - 配置对象
 * @returns {Promise<string>} DWR 响应文本
 */
export async function fetchTagPageByDWR(tagName, sort = 'new', getNum = 20, gotNum = 0, lastTimestamp = null, config = {}) {
  const url = 'https://www.lofter.com/dwr/call/plaincall/TagBean.search.dwr'
  ...
  const params = new URLSearchParams({
    ...
    'c0-param5': 'number:0',
    'c0-param6': `number:${getNum}`,
    'c0-param7': `number:${gotNum}`,
    'c0-param8': `number:${lastTimestamp || Date.now()}`,
    'batchId': '870178'
  })
  ...
  if (!response.ok) {
    throw new Error(`DWR 请求失败: HTTP ${response.status}`)
  }
