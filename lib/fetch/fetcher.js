 ...
 * @param {string} [sort='new'] - 排序方式 (new/hot)
 * @param {number} [offset=0] - 分页偏移量
 * @param {LofterConfig} [config={}] - 配置对象
 * @returns {Promise<object>} API JSON 响应的 response 字段
 */
export async function fetchBlogHomePageByAPI(blogName, sort = 'new', offset = 0, config = {}) {
  const url = 'https://api.lofter.com/v2.0/blogHomePage.api?product=lofter-android-7.9.7.2'
  const body = new URLSearchParams({
    blogdomain: `${blogName}.lofter.com`,
    offset: String(offset),
    method: 'getPostLists',
    limit: '100',
    sort
  ...
  if (!response.ok) {
    throw new Error(`博主主页 API 请求失败: HTTP ${response.status}`)
  }

  const json = await response.json()
  if (!json.response) {
    throw new Error('博主主页 API 返回数据格式异常')
  }
